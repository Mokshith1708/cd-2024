...
extern FILE *yyin;
void is_valid_date(int day, const char* month, int year);
...
s: D '-' M '-' Y { is_valid_date($1, $3, $5) ;} ';' | error ';' {yyerrok; yyclearin; printf("Rejected - Invalid Format \n") ;} ;
...
void is_valid_date(int day, const char* month, int year) {
    ...
    if (month_index == -1) {
         printf("%d-%s-%d - Rejected - Invalid Month\n",day,month,year);
        return ; 
    }

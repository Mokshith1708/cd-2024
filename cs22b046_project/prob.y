...
comp:
    ...
    ;
A : A ',' IDENTIFIER ASSIGN expr { sprintf(var_list[var_count],"%s",$3); var_list_brac[var_count] = 1;  var_count++; 
    ...
    |A ',' IDENTIFIER { sprintf(var_list[var_count],"%s",$3);var_list_brac[var_count] = 0;  var_count++; }
    | IDENTIFIER { sprintf(var_list[var_count],"%s",$1); var_list_brac[var_count] = 0;var_count++; 
                       }
    | IDENTIFIER ASSIGN expr { sprintf(var_list[var_count],"%s",$1); var_list_brac[var_count] = 1;  var_count++; 
            char* t = gen_label();
...
expr:
    expr '+' expr { //printf("hi +\n");
                    ...
                    char* label = new_temp();
                    add_tac("%s = %s + %s\n", label, a1, a2);
                    $$ = createNode(label, NULL, NULL, NULL,t);
                    }
                   }
  | expr '-' expr { //printf("hi -\n");
                      if(!error_flag)
  ...
  | '(' expr ')'   {// printf("hi (\n"); 
                       if(!error_flag)
                        {  Node* temp = createNode($2->value,$2->trueList,$2->falseList,NULL,$2->type);
                        $$ = temp;}
                        }
...
N: {$$ = tac_count; char *dummy1 = "goto "; add_tac("%s",dummy1); };
M: {$$ = tac_count;};
O:  {
...
term: UM IDENTIFIER  {
                        if(!error_flag)
                       {
                       if(checkSymbolExistsInAnyTable($2) == NULL){
                                printf("error : var ' %s ' is not declared in the scope\n",$2); error_flag = 1;
                            }
                        else{
                       char* label = new_temp();
                       add_tac("%s = -%s\n", label, $2);
                       Node* temp = createNode(label,NULL,NULL,NULL,checkSymbolExistsInAnyTable($2)->type);
                       $$ = temp;
                        }
                       }
     ...
     | IDENTIFIER  {
                        if(!error_flag)
                       { 
                            if(checkSymbolExistsInAnyTable($1) == NULL){
                                printf("error : var ' %s ' is not declared in the scope\n",$1); error_flag = 1;
                            }
                           else{
                         Node* temp = createNode($1,NULL,NULL,NULL,checkSymbolExistsInAnyTable($1)->type);
                         $$ = temp;
                           }
     ...
     | UM DOUBLE IDENTIFIER 
                           {    if(!error_flag)
                              {
                                if(checkSymbolExistsInAnyTable($3) == NULL){
                                printf("error : var ' %s ' is not declared in the scope\n",$3); error_flag = 1;
                            }
                               else {
                                 char* label = new_temp();
                                 ...
                                 char* new1 = new_temp();
                                   ...
                                   add_tac("%s = -%s\n",label,new);
                                   add_tac("%s = %s\n",$3, new1);
                                   Node* temp = createNode(label,NULL,NULL,NULL,checkSymbolExistsInAnyTable($3)->type);
                                   $$ = temp;
                            ...
                            else
                            {
                                ...
                                if(strcmp($2->value, "--") == 0)
                                 {  char* new1 = new_temp();
                                   ...
                                   add_tac("%s = %s - 1\n", new1, $1);
                                   add_tac("%s = %s\n", $1, new1);
                                   Node* temp = createNode(label,NULL,NULL,NULL,checkSymbolExistsInAnyTable($1)->type);
                                   $$ = temp;
                                 }
                                 else
                                 { char* new1 = new_temp();
